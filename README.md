...
- [ ] 최종 승패 출력
    - [ ] 플레이어들의 승패 여부
        ...
        - [ ]`[예외]` 플레이어와 딜러 모두 Bust 이면 플레이어가 패배한다. 
        - [ ]`[예외]` 플레이어가 Bust 이면 패배이다. 
        - [ ]`[예외]` 딜러와 플레이어 점수가 같으면 무승부이다.
    - [ ] 딜러의 전적
        - [ ] 딜러 점수가 21 이하이고, 플레이어 점수보다 크면 승리한다.
        - [ ] 딜러 점수가 21 이하이고, 플레이어 점수보다 작으면 패배한다. 
        - [ ]`[예외]` 플레이어와 딜러 모두 Bust 이면 딜러가 승리한다.
        - [ ]`[예외]` 딜러가 Bust 이면 패배이다. 
        - [ ]`[예외]` 딜러와 플레이어 점수가 같으면 무승부이다
...
- [ ] 최종 점수를 비교해 승패를 결정한다.
- [ ] 딜러가 Bust 이고, 플레이어가 정상이면 플레이어가 승리한다.
- [ ] 딜러가 Bust 이고, 플레이어도 Bust 이면 플레이어가 패배한다.
- [ ] 딜러가 BlackJack 이고 플레이어가 BlackJack 이면 무승부다.
